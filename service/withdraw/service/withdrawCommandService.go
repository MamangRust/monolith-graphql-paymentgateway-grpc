...
import (
	...
	mencache "github.com/MamangRust/monolith-graphql-payment-gateway-withdraw/redis"
	"github.com/MamangRust/monolith-graphql-payment-gateway-withdraw/repository"
...
func (s *withdrawCommandService) Create(ctx context.Context, request *requests.CreateWithdrawRequest) (*db.UpdateWithdrawStatusRow, error) {
	...
	go func() {
		...
		if s.kafka != nil {
			...
			s.logger.Warn("kafka is nil, skipping email notification", zap.Int("withdraw_id", int(updatedWithdraw.WithdrawID)))
		}
	}()
