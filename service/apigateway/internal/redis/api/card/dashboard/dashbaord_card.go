...
func (c *cardDashboardByCardNumberCache) GetDashboardCardCardNumberCache(ctx context.Context, cardNumber string) (*model.APIResponseDashboardCardNumber, bool) {
	key := fmt.Sprintf(cacheKeyDashboardCardNumber, cardNumber)
	result, found := cache.GetFromCache[model.APIResponseDashboardCardNumber](ctx, c.store, key)
...
func (c *cardDashboardByCardNumberCache) SetDashboardCardCardNumberCache(ctx context.Context, cardNumber string, data *model.APIResponseDashboardCardNumber) {
	if data == nil {
		return
	}
