...
type WithdrawCommandErrorHandler interface {
	...
	// Returns:
	//   - A WithdrawResponse with error details and a standardized ErrorResponse.
	HandleRestoreWithdrawError(err error, method, tracePrefix string, span trace.Span, status *string, fields ...zap.Field) (*response.WithdrawResponseDeleteAt, *response.ErrorResponse)
