...
// Returns:
//   - A WithdrawResponse with error details and a standardized ErrorResponse.
func (w *withdrawCommandError) HandleRestoreWithdrawError(err error, method, tracePrefix string, span trace.Span, status *string, fields ...zap.Field) (*response.WithdrawResponseDeleteAt, *response.ErrorResponse) {
	return handleErrorRepository[*response.WithdrawResponseDeleteAt](w.logger, err, method, tracePrefix, span, status, withdraw_errors.ErrFailedRestoreWithdraw, fields...)
}
