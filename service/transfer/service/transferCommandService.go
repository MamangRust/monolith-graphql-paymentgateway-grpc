...
import (
	...
	"github.com/MamangRust/monolith-graphql-payment-gateway-shared/errorhandler"
	...
	mencache "github.com/MamangRust/monolith-graphql-payment-gateway-transfer/redis"
	"github.com/MamangRust/monolith-graphql-payment-gateway-transfer/repository"
	"go.uber.org/zap"
)
...
func (s *transferCommandService) CreateTransaction(ctx context.Context, request *requests.CreateTransferRequest) (*db.UpdateTransferStatusRow, error) {
	...
	go func() {
		...
		if s.kafka != nil {
			...
			s.logger.Warn("kafka is nil, skipping email notification", zap.Int("transfer_id", int(updatedTransfer.TransferID)))
		}
	}()
