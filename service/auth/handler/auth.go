...
import (
	...
	"github.com/MamangRust/monolith-graphql-payment-gateway-auth/service"
	...
	protomapper "github.com/MamangRust/monolith-graphql-payment-gateway-shared/mapper/auth"
...
func (s *authHandleGrpc) ResetPassword(ctx context.Context, req *pb.ResetPasswordRequest) (*pb.ApiResponseResetPassword, error) {
	...
	_, err := s.passwordResetService.ResetPassword(ctx, request)
	if err != nil {
...
func (s *authHandleGrpc) parseValidationErrors(err error) []errors.ValidationError {
...
func (s *authHandleGrpc) getValidationMessage(fe validator.FieldError) string {
	switch fe.Tag() {
	case "required":
